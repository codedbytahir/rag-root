        ...
        <div className="flex max-w-[1280px] w-full px-4 sm:px-10 py-4 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {!user ? (
              ...
              <Link href="/dashboard" className="flex cursor-pointer items-center justify-center rounded-lg h-9 px-5 bg-[#39E079] hover:bg-[#0bcbcb] text-[#111818] text-sm font-bold transition-all shadow-[0_0_15px_rgba(13,242,242,0.3)] hover:shadow-[0_0_25px_rgba(13,242,242,0.5)] font-sans">
                Dashboard
              </Link>
            )}
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#0f1515] border-b border-white/5 p-6 flex flex-col gap-4 shadow-2xl">
           ...
           {!user ? (
             ...
             <Link href="/dashboard" onClick={() => setIsOpen(false)} className="text-gray-300 text-lg font-sans hover:text-[#39E079]">Dashboard</Link>
           )}
        </div>
      )}
    </nav>
